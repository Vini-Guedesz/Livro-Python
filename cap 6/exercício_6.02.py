"""
Números favoritos: Use um dicionário para armazenar os números favoritos
de algumas pessoas. Pense em cinco nomes e use-os como chaves em seu
dicionário. Pense em um número favorito para cada pessoa e armazene cada
um como um valor em seu dicionário. Exiba o nome de cada pessoa e seu
número favorito. Para que seja mais divertido ainda, faça uma enquete com
alguns amigos e obtenha alguns dados reais para o seu programa.
"""
num_favs = {
    'ricardo' : 5,
    'cezar' : 7,
    'igor' : 9,
    'arthur' : 3,
    'vini' : 7
}
print('o número favorito do ricardo é: ' + str(num_favs['ricardo']))
print('o número favorito do cezar é: ' + str(num_favs['cezar']))
print('o número favorito do igor é: ' + str(num_favs['igor']))
print('o número favorito do arthur é: ' + str(num_favs['arthur']))
print('o número favorito do vini é: ' + str(num_favs['vini']))